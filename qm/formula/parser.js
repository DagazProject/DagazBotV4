 ...
 */
function isTokenBinaryOperator(presedence, candidate) {
    switch (presedence) {
...
function createReaderClassSkipWhitespaces(reader) {
    let currentToken;
    const readNext = () => {
        currentToken = reader();
...
function parseExpression(readerFunc) {
    ...
    function readParenExpression() {
        ...
        if (reader.current().kind === "identifier") {
            ...
            if (!paramRegexpMatch) {
                throw new Error(`Unknown parameter '${reader.current().text}' at ${reader.current().start}`);
            }
            ...
            if (reader.current().kind !== "close paren token") {
                throw new Error(`Expected ], but got '${reader.current().text}' at ${reader.current().start}`);
            }
            reader.readNext();
            return exp;
        }
            ...
            while (true) {
                ...
                if (reader.current().kind === "dotdot token") {
                    ...
                    throw new Error(`Unexpected token inside paren '${reader.current().text}' pos=${reader.current().start} `);
                }
            }
            const expr = {
                type: "range",
    ...
    function readPrim() {
        ...
        if (primStartToken.kind === "numeric literal") {
            ...
            if (reader.current().kind !== "close brace token") {
                throw new Error(`Expected close brace token but got ${reader.current().text} at ${reader.current().start}`);
            }
            reader.readNext();
            return expr;
        }
            ...
            if (reader.current().kind === "end token") {
                ...
                throw new Error(`Expecting primary value at ${reader.current().start} but got '${reader.current().text}' kind=${reader.current().kind}`);
            }
        }
    }
    function readExpr(currentPriority = MAX_PRECEDENCE) {
    ...
    const expr = readExpr();
    if (reader.current().kind !== "end token") {
        throw new Error(`Unexpected data at ${reader.current().start}: '${reader.current().text}' kind=${reader.current().kind}`);
    }
    return expr;
}
